...
const pricesArrayElements = prices.map((price) => {
  ...
  */
  ...
  //Требования: 
Отображение карточек с данными должно происходить при нажатии на кнопку “Get Data”, при этом кнопка должна исчезать после отображения карточек

Данные пользователей нужно взять из массива объектов (данные будут предоставлены)

При работе необходимо использовать методы массивов

Имя и фамилия должны являться заголовком карточки, все остальные данные обычными элементами на ваше усмотрение
